...
public enum HeroState {Running=0, TurningLeft, TurningRight, Jumping, Shooting, Dying};
public enum TurnDirection {Left=-90, Right=90};
...
public class HeroController : MonoBehaviour {
	...
	private HeroState _state = HeroState.Running;
	...
	void Start(){
		...
		StartCoroutine(HeroStateMachine());
	}


	private IEnumerator HeroStateMachine(){
		while ( !_isDead ){
			...
			switch (_state){
				case HeroState.Running:
					break;
				case HeroState.TurningLeft:
					TurnHero(TurnDirection.Left);
					break;
				case HeroState.TurningRight:
					TurnHero(TurnDirection.Right);
					break;
			...
			SetState(HeroState.Running);
		}
	}


	private void ProcessMovement(){
		// TILTING (strafe the character)
		...
		if (_isGrounded && _isJumping){
			_isJumping = false;
			StartCoroutine(_heroAnimations.DidLand() );
		}

	}


	private void TurnHero(TurnDirection direction){
